%%% Calibration of the 2025 and 2027 options %%%
...
% For actual lsqnonlin parameters we use Npow = 18 and A = 1000
...
distance_fmin = @(p) sum(sum(abs(model_prices_2025(p) - market_prices_2025))) ...
                    + sum(sum(abs(model_prices_2027(p) - market_prices_2027))) ...
                    + (p(4)^2*p(1)^2*p(3) + 2*p(4)*p(2)*p(3) - 1 > 0)*1e16 ... 
                    + (p(1) < 0)*1e16 + (p(3) < 0)*1e16; % fmincon/fminsearch;
distance_lsq = @(p) [model_prices_2025(p) - market_prices_2025; ...
                     model_prices_2027(p) - market_prices_2027];
                 
% fminsearch                
tic
params1_2027 = fminsearch(distance_fmin, p0);
t1_2027 = toc;

% lsqnonlin
options = optimset('MaxFunEval', 10000); % otherwise it stops prematurely 
tic
params2_2027 = lsqnonlin(distance_lsq, p0, [], [], options);
t2_2027 = toc;

% fmincon
tic
params3_2027   = fmincon(distance_fmin, p0, [], [], [], [], [], [], NONLCON);
t3_2027 = toc;

% RMSEs 
N  = numel(market_prices_2025) + numel(market_prices_2027);
rmse1 = sqrt( 1/N * sum(sum((model_prices_2025(params1_2027) - market_prices_2025).^2)) + ...
              1/N * sum(sum((model_prices_2027(params1_2027) - market_prices_2027).^2)) );
rmse2 = sqrt( 1/N * sum(sum((model_prices_2025(params2_2027) - market_prices_2025).^2)) + ...
              1/N * sum(sum((model_prices_2027(params2_2027) - market_prices_2027).^2)) );
rmse3 = sqrt( 1/N * sum(sum((model_prices_2025(params3_2027) - market_prices_2025).^2)) + ...
              1/N * sum(sum((model_prices_2027(params3_2027) - market_prices_2027).^2)) );
